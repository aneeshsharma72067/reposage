                      ...
                      {analysisRuns.map((run, index) => {
                        ...
                        return (
                          <motion.article
                            ...
                            role="link"
                            tabIndex={0}
                            aria-label={`Open analysis run ${run.id}`}
                            onClick={() => router.push(runDetailHref)}
                            onKeyDown={(event: ReactKeyboardEvent<HTMLElement>) => {
                              if (event.key === 'Enter' || event.key === ' ') {
                                event.preventDefault();
                                router.push(runDetailHref);
                              }
                            }}
                            className="cursor-pointer rounded-xl border border-white/10 bg-black/25 px-4 py-3 backdrop-blur-md transition hover:border-cyan-300/25 hover:bg-black/35"
                          >
